...
use App\Http\Requests\Prompt\StoreCompletionPromptGenerateRequest;
...
class CompletionPromptGenerateController extends AbstractPromptGenerateController
{

    /**
     * GPT Completion Prompt 를 generate 합니다
     * @OA\Post(
     *     path="/api/prompt/{prompt}/generate/completion",
     ...
     *         @OA\Examples(example="She no went to the market.", value="She no went to the market.", summary="She no went to the market."),
     *     ),
     *     @OA\Parameter(
     ...
     * @throws \Throwable
     */
    public function store(Prompt $prompt, StoreCompletionPromptGenerateRequest $request)
    {
     ...
     * @OA\Post(
     *     path="/api/prompt/{prompt}/generate/expect/completion",
     ...
     *                 @OA\Schema(ref="#/components/schemas/Result"),
     *             },
     *             @OA\Property(property="data", type="float", example="0.0001"),
     *         ),
     *      ),
     * )
     *
     * @throws \Throwable
     */
    public function expectPoint(Prompt $prompt, StoreCompletionPromptGenerateRequest $request)
    {
        $validated = $request->validated();
        $callback = function () use ($validated, $prompt) {
            return ResponseTemplate::toJson(
